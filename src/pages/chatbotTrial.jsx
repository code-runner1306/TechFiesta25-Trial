...
const Chatbot = () => {
  ...
  return (
    <div className="relative min-h-screen flex flex-col items-center justify-center p-6 bg-cover bg-center bg-no-repeat"
  style={{ backgroundImage: `url('https://cdn.pixabay.com/photo/2020/09/28/16/29/leaves-5610361_1280.png')` }}>
  ...
  <div className="relative z-10 w-full max-w-xl sm:max-w-2xl text-center">
    <h1 className="text-3xl sm:text-4xl font-extrabold text-sky-400 drop-shadow-lg">🔵 Saathi AI - Safety Chatbot</h1>
  ...
  <div className="relative z-10 w-full max-w-xl sm:max-w-2xl mt-6">
    <div className="w-full p-6 sm:p-8 rounded-3xl border border-gray-700 bg-gradient-to-b from-gray-900 to-gray-800 shadow-lg">
      <h2 className="text-2xl font-bold text-sky-400 text-center mb-4">💬 Saathi AI Chat</h2>

      {/* Chat Display Section */}
      <div ref={chatContainerRef} className="h-72 sm:h-96 overflow-y-auto border border-gray-600 rounded-lg p-4 bg-gray-700">
        {chatHistory.length === 0 ? (
          <p className="text-gray-400 text-center italic">No messages yet...</p>
        ) : (
          chatHistory.map((message, index) => {
            const formattedMessage = message.replace("User:", "").replace("Saathi AI:", "");
            
            // Split message into parts based on asterisk sections
            const parts = formattedMessage.split(/(\*\*.*?\*\*|\*.*?\*)/g).filter(Boolean);
            ...
            return (
              ...
              >
                {parts.map((part, partIndex) => {
                  ...
                  if (isDoubleAsterisk) {
                    return (
                      <h2 key={partIndex} className="text-lg font-bold mb-2 border-b border-white/20 pb-1">
                        {part.replace(/\*\*/g, '')}
                      </h2>
                    );
                  } else if (isSingleAsterisk) {
                    return (
                      <h3 key={partIndex} className="text-base font-extrabold mb-1">
                        {part.replace(/\*/g, '')}
                      </h3>
                    );
                  } else {
                    return <span key={partIndex} className="inline-block mb-1">{part}</span>;
                  }
                })}
              </div>
            );
        ...
        {loading && (
          <p className="text-gray-400 text-center animate-pulse">Saathi AI is typing...</p>
        )}
      </div>
