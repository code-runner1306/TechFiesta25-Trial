...
const OrderProgress = ({ steps }) => {
  ...
  const getProgressBarGradient = () => {
    if (steps === 0) return 'bg-gradient-to-r from-red-500 to-red-400';
    if (steps === 1) return 'bg-gradient-to-r from-red-500 via-yellow-500 to-yellow-400';
    ...
    <div className="w-64 px-2 py-4">
      <div className="relative">
        ...
        <div 
          ...
          style={{ width: `${(steps / (steps1.length - 1)) * 100}%` }}
        />

        {/* Steps Container */}
        <div className="relative flex justify-between">
          {steps1.map((s, idx) => {
            const isCompleted = idx <= steps;
